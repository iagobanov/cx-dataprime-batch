...
- `examples/simple_count.dp` — count errors by severity across an application
- `examples/error_rate_by_region.dp` — error rate and response time grouped by cloud region
- `examples/extract_and_aggregate.dp` — parse HTTP method/path from logs and aggregate top endpoints
- `examples/join_users_with_errors.dp` — correlate user errors with login activity using a join

Adapt the filter values, field names, and subsystem names to match your environment.
